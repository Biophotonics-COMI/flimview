...
turbo_r = ListedColormap(turbo_colormap_data[::-1])


def plot2d(data, title='', extra_data=None, extra_title='', **kwargs):
    datasets=[]
    if extra_data is None:
        ...
        figs = 2
        datasets=[data, extra_data]
        titles=[title, extra_title]
    fig.subplots_adjust(hspace=0.2, wspace=0.2)
    for i in range(figs):
        ax = fig.add_subplot(1, figs, i+1)# plt.gca()
        ims = ax.imshow(datasets[i], origin='lower', **kwargs)
...
def plotFit(Ffit, Fbin, ix, iy):
    t = Fbin.t
    m = Fbin.data[iy,ix,:]
    tf,yf,ym,ts = flim.cleanCurve(t,m,norm=True, threshold=0.02)
    pp = []
    pc = []
    for k in Ffit.parameters:
        temp = getattr(Ffit, k)
        temp_err = getattr(Ffit, k+'_err')
        pp.append(temp[iy,ix])
        pc.append(temp_err[iy,ix])
    chi2 = Ffit.chi2[iy,ix]
    fig = plt.figure(figsize=(14,10))
    grid = plt.GridSpec(14, 10, wspace=0.1, hspace=0.1)
    ax0 = fig.add_subplot(grid[0:7,0:7 ])
    ax1 = fig.add_subplot(grid[7:10,0:7 ], sharex=ax0)
    ax2 = fig.add_subplot(grid[7:10,7:10 ], sharey=ax1)
    ax3 = fig.add_subplot(grid[0:7,7:10 ])
    ax0.plot(t,m,'.')
    ax0.plot(tf+ts,Ffit.model(tf, *pp)*ym)
    title = flim.printModel(Ffit.model, pp, np.diag(pc)**2, chi2, oneliner=True)
    if Ffit.Fcube.mask[iy,ix]:
        title='MASKED'
    _=ax0.set_title(title, fontsize=10)
    ax1.plot(tf+ts,(yf-Ffit.model(tf, *pp))*100./yf,'-')
    ax1.axhline(0, c='red', ls=':')
    ax1.set_xlabel('time [ns]', fontsize=14)
    ax0.axes.get_xaxis().set_visible(False) 
    ax1.set_ylabel('residuals [%]', fontsize=12 )
    ax0.set_ylabel('Intensity', fontsize=12 )
    ax2.hist((yf-Ffit.model(tf, *pp))*100./yf, bins=20, orientation="horizontal");
    ax2.axes.get_yaxis().set_visible(False) 
    ax2.axes.get_xaxis().set_visible(False)
    ax2.axhline(0, c='red', ls=':')
    ax3.axes.get_yaxis().set_visible(False) 
    ax3.axes.get_xaxis().set_visible(False)
    ax3.imshow(Fbin.intensity, origin='lower', cmap=turbo)
    ax3.axhline(iy, c='black', ls='-', lw=1.2)
    ax3.axvline(ix, c='black', ls='-', lw=1.2)
